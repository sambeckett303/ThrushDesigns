Vue.component('product-details',
{
	...
	template:
		...
		+			    '<input data-min="1" v-model="quantity" data-max="0" type="text" name="quantity" value="1" readonly="true">'
		+			    '<div class="quantity-selectors-container">'
		+		    		'<div class="quantity-selectors">'
		+		        		'<button type="button" class="increment-quantity" aria-label="Add one" @click="addOne"><span>&#43;</span></button>'
		+		        		'<button type="button" class="decrement-quantity" aria-label="Subtract one" @click="subtractOne" :disabled="quantity == 1"><span>&#8722;</span></button>'
		+		      		'</div>'
		+				'</div>'
		+			'</div>'
		+			'<div @click="addToCart" style="margin-top: 20px;" class="thrushButton">ADD TO MY CART</div>'
		+		'</div>'
		+	'</div>'
		+	'<modal v-if="loading">'
